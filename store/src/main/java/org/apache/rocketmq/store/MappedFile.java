...
public class MappedFile extends ReferenceResource {
    ...
    /**当前文件的提交指针 如果开启transientStorePoolEnable则数据先会存储到TransientStorePool中，
     * 然后再提交到内存映射ByteBuffer中，再刷写到磁盘*/
    protected final AtomicInteger committedPosition = new AtomicInteger(0);
    /**当前文件的刷盘指针，指针前的数据持久化到磁盘*/
    ...
    /**
     * 堆外内存ByteBuffer,如果不为空数据先会存储到该buffer中，然后提交到MappedFile对应的MappedByteBuffer。
     ...
     *
     * @param msg 消息
     * @param cb 追加消息回调（消息具体哪些字段需要添加到文件中都由它控制）
     * @return
     */
    ...
    public AppendMessageResult appendMessagesInner(final MessageExt messageExt, final AppendMessageCallback cb) {
        ...
        if (currentPos < this.fileSize) {
            //transientStorePoolEnable为true时，writeBuffer不为空
            //writeBuffer不为空是使用writeBuffer做切片 否则使用mappedByteBuffer做内存切片
    ...
    public int flush(final int flushLeastPages) {
        ...
        if (this.isAbleToFlush(flushLeastPages)) {
            if (this.hold()) {
                ...
                try {
                    //会把数据提交到 mappedByteBuffer 或者文件的 fileChannel，但是不会两者都提交
    ...
    protected void commit0(final int commitLeastPages) {
        ...
        if (writePos - lastCommittedPosition > commitLeastPages) {
            try {
                //通过ByteBuffer切片创建一个共享缓存区 把writeBuffer中上次提交位置到最新写入位置的数据提交到文件的FileChannel中
    ...
    private boolean isAbleToFlush(final int flushLeastPages) {
        ...
        //flushLeastPages > 0时需要脏页数量达到flushLeastPages才刷盘
        if (flushLeastPages > 0) {
            return ((write / OS_PAGE_SIZE) - (flush / OS_PAGE_SIZE)) >= flushLeastPages;
        }
        //flushLeastPages = 0 时强制刷盘
        return write > flush;
    }
    ...
    protected boolean isAbleToCommit(final int commitLeastPages) {
        ...
        //如果commitLeastPages大于0 则用 写指针和刷盘指针的差值除以页面大小 得到脏页数量
    ...
    /**
     * 以pos为起始位置 创建一个 size大小的切片ByteBuffer
     * @param pos  起始位置
     * @param size 大小
     * @return
     */
