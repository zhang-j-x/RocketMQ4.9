...
public class RouteInfoManager {
    ...
    public RegisterBrokerResult registerBroker(
        ...
        try {
            try {
                ...
                if (null == brokerNames) {
                    //如果为空 则为新集群 key 集群名称 value 集群broker列表
                ...
                //获取broker对应信息 brokerData
                BrokerData brokerData = this.brokerAddrTable.get(brokerName);
                if (null == brokerData) {
                    //brokerData 为null则证明首次注册
                    registerFirst = true;
                    brokerData = new BrokerData(clusterName, brokerName, new HashMap<Long, String>());
                    //将封装后的brokerData加入到broker数据映射表
                ...
                //物理节点的角色发生变化了，需要将其从物理节点的映射表中剔除 如：节点1 之前 brokerId为1 是从节点，现在brokerId为0 是主节点
                //那么就要把是从节点那条数据剔除，并写入主节点的信息
                while (it.hasNext()) {
                ...
                //添加当期broker的BrokerLiveInfo，定时任务会扫描这个表，返回了上一次broker的心跳对象
    ...
    public void unregisterBroker(
        ...
        final String brokerName,
        final long brokerId) {
        //同时更新 topicQueueTable，brokerAddrTable，brokerLiveTable，filterServerTable元数据信息
        try {
            try {
                this.lock.writeLock().lockInterruptibly();
                //从brokerLiveTable移除
        ...
        try {
            try {
                ...
                if (queueDataList != null) {
                    ...
                    // 从brokerAddrTable 和 filterServerTable 获取对应信息
    ...
    public void scanNotActiveBroker() {
        ...
        while (it.hasNext()) {
            ...
            if ((last + BROKER_CHANNEL_EXPIRED_TIME) < System.currentTimeMillis()) {
                ...
                //next.getKey() broker地址
                // next.getValue().getChannel() 服务器与broker的通道
        ...
        if (channel != null) {
            try {
                try {
                    ...
                    //根据channel取brokerLiveTable获取对应的brokerAddr
